...
def randomint(nbits: int, rng: Optional[KeccakBasedCSPRNG] = None) -> int:
    
    if nbits <= 0:
        return 0
    if rng is None:
        rng = KeccakBasedCSPRNG()
    return rng.get_random_int(nbits)


def randomint_for_dh(p: int, rng: Optional[KeccakBasedCSPRNG] = None) -> int:

    if p <= 3:
        raise ValueError("p musi być >= 4, aby można było wygenerować a w [2, p-2].")
    ...
    max_val = p - 2
    while True:
        candidate = rng.get_random_int(nbits)
